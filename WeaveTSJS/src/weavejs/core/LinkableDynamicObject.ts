...
namespace weavejs.core
{
  ...
  import DynamicStateArray = weavejs.api.core.DynamicStateArray;
	...
	export class LinkableDynamicObject extends LinkableWatcher implements ILinkableDynamicObject, ICallbackCollection
	{
		...
		public setSessionState(newState:DynamicStateArray, removeMissingDynamicObjects:boolean):void
		{
			...
			try
			{
				...
				var dynamicState:TypedState = newState[0] as TypedState;
				var className:string = (dynamicState as TypedState).className;
				var objectName:string = (dynamicState as TypedState).objectName;
				var sessionState:SessionState = (dynamicState as TypedState).sessionState;
		...
		public get globalName():string
		{
			if (this._targetPath && this._targetPath.length == 1)
				return this._targetPath[0] as any as string;
			return null;
		}
		 ...
		 */
		private handleDeprecatedSessionState(newState:DynamicStateArray, removeMissingDynamicObjects:boolean):void
		{
			...
			while (i--)
			{
				...
				if ((item as TypedState).className == SessionManager.DIFF_DELETE)
				{
					// remove object if name matches
					if (this.globalName == ((item as TypedState).objectName || null)) // convert empty string to null
						this.target = null;
				}
				else
				{
