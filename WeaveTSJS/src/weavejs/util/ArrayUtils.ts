	...
	export class ArrayUtils
	{
		...
		public static union<T>(...arrays:T[][]):T[]
		{
			...
			for (var array of arrays || [])
			{
				for (var item of array || [])
				{
					...
					{
						_setLookup(item, ArrayUtils._lookupId);
						result.push(item);
					}
				}
			}
		...
		public static flatten<T>(source:T[]|T, destination:T[] = null):T[]
		{
			...
			for (var i:int = 0; i < (source as T[]).length; i++)
				if (Weave.IS(source[i], Array))
					ArrayUtils.flatten(source[i], destination);
				else
		 ...
		 */
		public static getItems(object:{[key:string]:any}|any[], keys:string[], output:{[key:string]:any}|any[] = null):{[key:string]:any}|any[]
		{
			if (!output)
		 ...
		 */
		public static createLookup(array:any[]|{[key:string]:any}, ...propertyChain:string[]):Map<any,string>
		{
