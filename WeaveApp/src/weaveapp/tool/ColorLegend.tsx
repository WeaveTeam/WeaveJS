...
import ColorRampComponent from "weaveapp/ui/ColorRampComponent";
import {BinningDefinitionSelector} from "weaveapp/editor/BinningDefinitionEditor";
import ColorRampEditor from "weaveapp/editor/ColorRampEditor";
...
export default class ColorLegend extends React.Component<IVisToolProps, IVisToolState> implements ILinkableObjectWithNewProperties, IVisTool, IInitSelectableAttributes
{
	...
	getSelectedBins():number[]
	{
		if (this.selectionKeySet)
			return _.uniq(this.selectionKeySet.keys.map((key:IQualifiedKey) => this.binnedColumn.getValueFromKey(key, Number)));
		return [];
	}
	...
	{
		if (this.probeKeySet)
			return _.uniq(this.probeKeySet.keys.map((key:IQualifiedKey) => this.binnedColumn.getValueFromKey(key, Number)));
		return [];
	}
