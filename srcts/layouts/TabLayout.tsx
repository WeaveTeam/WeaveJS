...
export default class TabLayout extends AbstractLayout<TabLayoutProps, {}> implements weavejs.api.core.ILinkableVariable
{
	...
	private switchPanelToActive=(indexFromTabsComponent:number):void=>
	{
		...
		this.setSessionState(state);
		InteractiveTour.isEnabled()?InteractiveTour.targetComponentOnClick(state.tabs[state.activeTabIndex].label) : null
	};
	...
	render():JSX.Element
	{
		...
		if (this.props.onAdd)
		{
			...
			{
				plusIcon = (
					<Dropdown style={{display: "flex"}} menu={this.props.onAdd as MenuItemProps[]} direction="upward"  >
