...
namespace weavejs.util
{
	export class StandardLib
	{
		 ...
		 */
		public static substitute(format:string, ...args:{toString:() => string}[]):string
		{
			if (format == null)
				return '';
			...
			{
				var str:string = split[i];
				if (StandardLib.argRef.test(str))
				{
					var j:int = str.indexOf("}");
					output += (args as any)[str.substring(0, j)];
		...
		private static as3SortOn(array:any[], fieldNames:string|string[], options:any):any[]
		{
			/* Normalize to arrays */
			...
			var result = _.sortByOrder(array, iteratees, orders);
		 ...
		 * @see Array#sortOn()
		 */
		public static sortOn<T>(array:T[], params:any, sortDirections:any = undefined, inPlace:boolean = true, returnSortedIndexArray:boolean = false):any[]
		{
			if (array.length == 0)
			...
			for (var p:int = 0; p < params.length; p++)
			{
				...
				if (Weave.IS(param, Array))
					...
					while (i--)
						StandardLib._sortBuffer[i][p] = param((array as any)[i]);
				else if (Weave.IS(param, Map) || Weave.IS(param, WeakMap))
					while (i--)
						StandardLib._sortBuffer[i][p] = param.get((array as any)[i]);
				else if (typeof param === 'object')
					while (i--)
						StandardLib._sortBuffer[i][p] = param[(array as any)[i]];
				else
					while (i--)
						StandardLib._sortBuffer[i][p] = (array as any)[i][param];
			...
			i = array.length;
			while (i--)
				array2[i] = array[values[i] as any];
		...
		private static guessSortMode(array:any[], itemProp:int):int
		{
			for (var item of array || [])
			{
				...
				if (value !== undefined)
					return Weave.IS(value, Number) || Weave.IS(value, Date) ? (Array as any).NUMERIC : 0;
			}
			return 0;
		}
		...
		public static getArrayType(a:any[]):GenericClass
		{
			if (a == null || a.length == 0 || a[0] == null)
				return null;
			var type:GenericClass = (Object(a[0]) as any).constructor;
			for (var item of a || [])
		...
		public static compare<T>(a:T, b:T, objectCompare:(a:T, b:T)=>int = null):int
		{
			...
			if (typeA == 'number')
				return StandardLib.numericCompare(a as any, b as any);
			if (typeA == 'string')
				return StandardLib.stringCompare(a as any, b as any);
			if (typeA != 'object')
				return 1;
			if (Weave.IS(a, Date) && Weave.IS(b, Date))
				return StandardLib.dateCompare(a as any, b as any);
			if (Weave.IS(a, Array) && Weave.IS(b, Array))
			{
				var an:int = (a as any).length;
				var bn:int = (b as any).length;
				if (an < bn)
					return -1;
				if (an > bn)
					return 1;
				...
				{
					c = StandardLib.compare((a as any)[i], (b as any)[i]);
					if (c != 0)
						return c;
				}
				return 0;
			...
			for (p in b)
			{
				...
				c = StandardLib.compare((a as any)[p], (b as any)[p]);
				if (c != 0)
					return c;
			}
