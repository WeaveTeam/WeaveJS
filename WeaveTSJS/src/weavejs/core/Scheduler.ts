...
namespace weavejs.core
{
	...
	export class Scheduler implements IScheduler, IDisposableObject
	{
		...
		private _currentFrameStartTime:int = Date.now(); // this is the result of JS.now() on the last ENTER_FRAME event.
			...
			while (true)
			{
				...
				if (countdown == 0 || now > pStop)
				{
					...
					for (var i:int = minPriority; i < this._priorityCallLaterQueues.length; i++)
						remaining += (this._priorityCallLaterQueues[i] as Array).length;
...
//				WeaveAPI.SessionManager.unassignBusyTask(args);
		...
		public callLater(relevantContext:Object, method:Function, parameters:Array = null):void
		{
			this._callLaterPriority(WeaveAPI.TASK_PRIORITY_IMMEDIATE, relevantContext, method, parameters);
		}
		
		private _callLaterPriority(priority:uint, relevantContext:Object, method:Function, parameters:Array = null):void
		{
			if (method == null)
			{
