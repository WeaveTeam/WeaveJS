...
namespace weavejs.util
{
  ...
  import TypedState = weavejs.api.core.TypedState;
	...
	export class DebugUtils
	{
		 ...
		 */
		public static debugTrace(...args:any[]):void
		{
		...
		public static debugId(object:Object):string
		{
			...
			var idString:string = DebugUtils.map_obj_id.get(object) as string;
			if (!idString)
			{
		 ...
		 */
		public static debugLookup(debugId:string = undefined):Object
		{
		...
		{
			DebugUtils.map_id_obj = new Map<string, Object>();
			DebugUtils.map_obj_id = new Map<Object, string>();
		...
		public static watchState(target:ILinkableObject = null, indent:string = null):void
		{
			if (!target)
			{
			...
			if (DynamicState.isDynamicStateArray(state))
			{
				...
				for (var key in state)
				{
					...
					if (DynamicState.isDynamicState(obj))
					{
						...
						var sessionState:any = obj.sessionState;
		 ...
		 */
		public static traverseStatePath(state:SessionState, path:(string)[]):SessionState
		{
			try
			{
				outerLoop: for (var i in path)
				{
					...
					if (DynamicState.isDynamicStateArray(state))
					{
						...
						{
							state = ((state as DynamicStateArray)[property as any as number] as TypedState).sessionState;
						}
						else
						{
							for (var key in state as UnTypedState)
							{
								var obj:TypedState = (state as UnTypedState)[key as string];
								if (obj.objectName == property)
								{
									state = obj.sessionState;
									continue outerLoop;
								}
							}
							return undefined;
						...
						state = (state as UnTypedState)[property];
				}
				return state;
			}
			catch (e)
...
//		}
		...
		public static replaceUnknownObjectsInState(stateToModify:SessionState, className:string = null):SessionState
		{
			if (DynamicState.isDynamicStateArray(stateToModify))
			{
				for (var key in stateToModify as DynamicStateArray)
				{
					var obj:TypedState = (stateToModify as DynamicStateArray)[key] as TypedState;
					if (!DynamicState.isDynamicState(obj))
						continue;
					...
					if (!classDef)
					{
						...
						obj.className = 'LinkableHashMap';
					}
					if (Weave.IS(classDef.prototype, ILinkableCompositeObject))
						obj.sessionState = DebugUtils.replaceUnknownObjectsInState(obj.sessionState, className);
				}
			}
			else if (!JS.isPrimitive(stateToModify))
			{
				...
				for (var key in stateToModify as UnTypedState)
				{
					var value = (stateToModify as UnTypedState)[key];
