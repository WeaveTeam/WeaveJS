...
namespace weavejs.util
{
	...
	export class ColorRamp extends LinkableVariable
	{
		...
		{
			super(null, this.verifyState, sessionState || this.getColorRampByName("Blues").colors);
		}
		...
		private validate():void
		{
			...
			for (var item of this.getSessionState() || [])
			{
				if (typeof item == 'object')
					_colorNodes.push(new ColorNode(item[ColorRamp.POSITION], item[ColorRamp.COLOR]));
				else
			...
			var positions:number[] = ArrayUtils.pluck(_colorNodes, ColorRamp.POSITION);
		...
		public getHexColors():number[]
		{
			var colors:number[] = this.getColors();
