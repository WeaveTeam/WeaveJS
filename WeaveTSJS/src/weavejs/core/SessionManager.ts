...
namespace weavejs.core
{
	...
	{
		public static debugUnbusy:boolean = false;
		
		public newLinkableChild<T>(linkableParent:Object, linkableChildType:Class<T>, callback:()=>void = null, useGroupedCallback:boolean = false):T
		{
			...
			var linkableChild:T = Weave.AS(new linkableChildType(), ILinkableObject);
			return this.registerLinkableChild(linkableParent, linkableChild, callback, useGroupedCallback);
		}
		
		public registerLinkableChild<T>(linkableParent:Object, linkableChild:T, callback:()=>void = null, useGroupedCallback:boolean = false):T
		{
			if (!Weave.isLinkable(linkableParent))
			...
			if (callback != null)
			{
				var cc:ICallbackCollection = this.getCallbackCollection(linkableChild);
				if (useGroupedCallback)
					cc.addGroupedCallback(Weave.AS(linkableParent, ILinkableObject), callback);
				else
					cc.addImmediateCallback(Weave.AS(linkableParent, ILinkableObject), callback);
			}
			...
			if (this.d2d_child_parent.get(linkableChild, linkableParent) === undefined)
			{
				...
				var parentCC:ICallbackCollection = this.getCallbackCollection(Weave.AS(linkableParent, ILinkableObject));
		...
		public newDisposableChild<T>(disposableParent:Object, disposableChildType:Class<T>):T
		{
			return this.registerDisposableChild(disposableParent, new disposableChildType());
		}
		
		public registerDisposableChild<T>(disposableParent:Object, disposableChild:T):T
		{
			if (!disposableParent)
		 ...
		 */
		public unregisterLinkableChild(parent:ILinkableObject, child:ILinkableObject):void
		{
			if (!parent)
			...
			this.getCallbackCollection(child).removeCallback(parent, this.getCallbackCollection(parent).triggerCallbacks);
		 ...
		 */
		public excludeLinkableChildFromSessionState(parent:ILinkableObject, child:ILinkableObject):void
		{
			if (parent == null || child == null)
			{
				console.error("SessionManager.excludeLinkableChildFromSessionState(): Parameters to this function cannot be null.");
				return;
			}
		 ...
		 */
		private _getRegisteredChildren(parent:ILinkableObject):ILinkableObject[]
		{
			return this.d2d_parent_child.secondaryKeys(parent);
		}

		public getOwner(child:Object):Object
		{
			return this.map_child_owner.get(child);
		}
		
		public getLinkableOwner(child:ILinkableObject):ILinkableObject
		{
			return Weave.AS(this.map_child_owner.get(child), ILinkableObject);
		}
		
		/**
		 * Cached WeaveTreeItems that are auto-generated when they are accessed
		 */
		private d2d_object_name_tree:Dictionary2D = new Dictionary2D(true, false, WeaveTreeItem);
		 ...
		 */
		public getSessionStateTree(root:ILinkableObject, objectName:string):WeaveTreeItem
		{
			if (!root)
				return null;
			var treeItem:WeaveTreeItem = this.d2d_object_name_tree.get(root, objectName);
			if (!treeItem.data)
			{
				treeItem.data = root;
				treeItem.children = this.getTreeItemChildren;
		...
		private getTreeItemChildren(treeItem:WeaveTreeItem):WeaveTreeItem[]
		{
			var object:ILinkableObject = treeItem.data as ILinkableObject;
			var children:Array = [];
			var names:Array;
			var childObject:ILinkableObject;
			var ignoreList = new WeakMap<any,any>();
			var lhm:ILinkableHashMap = Weave.AS(object, ILinkableHashMap);
			if (lhm)
			{
				names = lhm.getNames();
			...
			{
				var ldo:ILinkableDynamicObject = Weave.AS(object, ILinkableDynamicObject);
				if (ldo)
				{
		 ...
		 */
		public getTypedStateTree(root:ILinkableObject):Object
		{
			return this.getTypedStateFromTreeNode(this.getSessionStateTree(root, null));
		}
		private getTypedStateFromTreeNode(node:WeaveTreeItem, i:int = 0, a:Array = null):Object
		{
			var state:Object;
		 ...
		 */
		public addTreeCallback(relevantContext:Object, groupedCallback:Function, triggerCallbackNow:boolean = false):void
		{
			_treeCallbacks.addGroupedCallback(relevantContext, groupedCallback, triggerCallbackNow);
		}
		public removeTreeCallback(relevantContext:Object, groupedCallback:Function):void
		{
			_treeCallbacks.removeCallback(relevantContext, groupedCallback);
		}
		private _treeCallbacks:CallbackCollection = new CallbackCollection();

		public copySessionState(source:ILinkableObject, destination:ILinkableObject):void
		{
			var sessionState:Object = getSessionState(source);
			setSessionState(destination, sessionState, true);
		}
		
		private applyDiffForLinkableVariable(base:Object, diff:Object):Object
		{
		...
		public static /* readonly */ DEPRECATED_STATE_MAPPING:string = 'deprecatedStateMapping';
		public static /* readonly */ DEPRECATED_PATH_REWRITE:string = 'deprecatedPathRewrite';
		 ...
		 */
		public static traverseAndSetState(state:Object, mapping:Object, removeMissingDynamicObjects:boolean = true):void
		{
		...
		public setSessionState(linkableObject:ILinkableObject, newState:Object, removeMissingDynamicObjects:boolean = true):void
		{
		 ...
		 */
		private map_obj_getSessionStateIgnore:Object = new JS.WeakMap();
		
		public getSessionState(linkableObject:ILinkableObject):Object
		{
			if (linkableObject == null)
			{
			...
			else
			{
				...
				var property:ILinkableObject = null;
				var i:int;
				//console.log("getting session state for "+Weave.className(sessionedObject),"propertyNames="+propertyNames);
				...
				if (resultNames.length > 0)
				{
					...
					for (i = 0; i < resultNames.length; i++)
					{
						...
						//console.log("getState",Weave.className(sessionedObject),resultNames[i],result[resultNames[i]]);
					}
				}
			}
		...
		private static /* readonly */ LINKABLE_PROPERTIES:string = 'linkableProperties';
		 ...
		 */
		public getLinkablePropertyNames(linkableObject:ILinkableObject, filtered:boolean = false):Array
		{
			if (linkableObject == null)
			{
		 ...
		 */
		private map_child_owner:Object = new JS.WeakMap();
		/**
		 ...
		 */
		private d2d_owner_child:Dictionary2D = new Dictionary2D(true, false);
		/**
		 ...
		 */
		private d2d_child_parent:Dictionary2D = new Dictionary2D(true, false);
		/**
		 ...
		 */
		private d2d_parent_child:Dictionary2D = new Dictionary2D(true, false);
		
		public getLinkableDescendants(root:ILinkableObject, filter:Class = null):Array
		{
			var result:Array = [];
			if (root)
		...
		private internalGetDescendants(output:Array, root:ILinkableObject, filter:Class, ignoreList:Object, depth:int):void
		{
		...
		private map_task_stackTrace:Object = new JS.Map();
		private d2d_owner_task:Dictionary2D = new Dictionary2D(false, false);
		private d2d_task_owner:Dictionary2D = new Dictionary2D(false, false);
		private map_busyTraversal:Object = new JS.WeakMap(); // ILinkableObject -> Boolean
		private array_busyTraversal:Array = [];
		private map_unbusyTriggerCounts:Object = new JS.Map(); // ILinkableObject -> int
		private map_unbusyStackTraces:Object = new JS.WeakMap(); // ILinkableObject -> String
		
		private disposeBusyTaskPointers(disposedObject:ILinkableObject):void
		{
			d2d_owner_task.removeAllPrimary(disposedObject);
			d2d_task_owner.removeAllSecondary(disposedObject);
		}
		
		public assignBusyTask(taskToken:Object, busyObject:ILinkableObject):void
		{
		...
		public unassignBusyTask(taskToken:Object):void
		{
		 ...
		 */
		private unbusyTrigger(stopTime:int):number
		{
			while (map_unbusyTriggerCounts.size)
			{
				...
				for each (var owner_triggerCount:Array in entries)
				{
					...
					if (WeaveAPI.debugAsyncStack && debugUnbusy)
					{
						...
						console.log('Triggering callbacks because they have not triggered since owner has becoming unbusy:', owner);
		...
		public linkableObjectIsBusy(linkableObject:ILinkableObject):boolean
		{
		 ...
		 */
		private map_ILinkableObject_ICallbackCollection:Object = new JS.WeakMap();
		
		/**
		 * This maps an ICallbackCollection to the ILinkableObject associated with it.
		 */
		private map_ICallbackCollection_ILinkableObject:Object = new JS.WeakMap();

		public getCallbackCollection(linkableObject:ILinkableObject):ICallbackCollection
		{
			if (linkableObject == null)
				return null;
		...
		public getLinkableObjectFromCallbackCollection(callbackCollection:ICallbackCollection):ILinkableObject
		{
			return map_ICallbackCollection_ILinkableObject.get(callbackCollection) || callbackCollection;
		}
		
		public objectWasDisposed(object:Object):boolean
		{
			if (object == null)
				return false;
		...
		private map_disposed:Object = new JS.WeakMap();
		
		private static /* readonly */ DISPOSE:string = "dispose"; // this is the name of the dispose() function.

		public disposeObject(object:Object):void
		{
		...
		// FOR DEBUGGING PURPOSES
		private debugDisposedObject(disposedObject:ILinkableObject, disposedError:Error):void
		{
		 ...
		 * For debugging only.
		 */
		public _getPaths(root:ILinkableObject, descendant:ILinkableObject):Array
		{
			var results:Array = [];
		 ...
		 * internal use only
		 */
		private _getChildPropertyName(parent:ILinkableObject, child:ILinkableObject):string
		{
		...
		public getPath(root:ILinkableObject, descendant:ILinkableObject):Array
		{
			if (!root || !descendant)
				return null;
		...
		private _getPath(tree:WeaveTreeItem, descendant:ILinkableObject):Array
		{
		...
		public getObject(root:ILinkableObject, path:Array):ILinkableObject
		{
			if (!path)
				return null;
		...
		private getObjectFromDeprecatedPath(mapping:Object, path:Array, startAtIndex:int):ILinkableObject
		{
		 ...
		 */
		private d2d_lhs_rhs_setState:Dictionary2D = new Dictionary2D(true, true);
		public linkSessionState(primary:ILinkableObject, secondary:ILinkableObject):void
		{
		...
		public unlinkSessionState(first:ILinkableObject, second:ILinkableObject):void
		{
		...
		public static /* readonly */ DIFF_DELETE:string = 'delete';
		
		public computeDiff(oldState:Object, newState:Object):*
		{
		...
		public combineDiff(baseDiff:Object, diffToAdd:Object):Object
		{
			// convert undefined params to null params
			if (baseDiff == null)
				baseDiff = null;
