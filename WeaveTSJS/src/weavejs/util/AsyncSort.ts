...
namespace weavejs.util
{
	...
	export class AsyncSort<T> implements ILinkableObject
	{
		...
		private static _immediateSorter:AsyncSort; // used by sortImmediately()
		...
		public static sortImmediately<T>(array:T[], compareFunction:(a:T, b:T)=>int = null):void
		{
			...
			{
				AsyncSort._immediateSorter = new AsyncSort();
			...
			var sorter:AsyncSort = AsyncSort._immediateSorter;
		...
		*/
		...
		private static initTestArrays(testType:int):void
		{
			if (testType != AsyncSort._testType)
			{
				...
				for (var n:uint of [0,1,2,3,4,5,50,3000,6000,12000,25000,50000])
				{
		...
		public static test(compare:(a:any, b:any)=>int, testType:int = 0):void
		{
			...
			for (var _array of AsyncSort._testArrays || [])
			{
				...
				if (compare === null)
					...
					array1.sort(compare);
				console.log('Array.sort', array1.length, 'numbers;', (JS.now() - start) / 1000, 'seconds;', AsyncSort._debugCompareCount ? (AsyncSort._debugCompareCount+' comparisons') : '');
				...
				AsyncSort.verifyNumbersSorted(array2);
			}
		}
		public static testSortOn(compare:(a:string|number, b:string|number)=>int, testType:int = 0):void
		{
			...
			for (var _array of AsyncSort._testArraysSortOn)
			{
				...
				*/
				...
				console.log('StdLib sortOn', array3.length, 'numbers;', (JS.now() - start) / 1000, 'seconds;', AsyncSort._debugCompareCount ? (AsyncSort._debugCompareCount+' comparisons') : '');
				
				start = JS.now();
				AsyncSort._debugCompareCount = 0;
				StandardLib.sortOn(array4, ['value']);
				console.log('StdLib sortOn[]', array4.length, 'numbers;', (JS.now() - start) / 1000, 'seconds;', AsyncSort._debugCompareCount ? (AsyncSort._debugCompareCount+' comparisons') : '');

				start = Date.now();
				AsyncSort._debugCompareCount = 0;
				AsyncSort.sortImmediately(array2, AsyncSort._debugCompareCounter);
				//trace('Merge Sort', n, 'numbers;', _immediateSorter.elapsed / 1000, 'seconds;',_debugCompareCount,'comparisons');
				console.log('Merge SortOn', array2.length, 'numbers;', (JS.now() - start) / 1000, 'seconds;', AsyncSort._debugCompareCount ? (AsyncSort._debugCompareCount+' comparisons') : '');
