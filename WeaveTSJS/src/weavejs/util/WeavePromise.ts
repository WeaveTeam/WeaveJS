	...
	export class WeavePromise<T> implements IDisposableObject
	{
		...
		constructor(relevantContext:Object, resolver:( resolve:(value?:T)=>void, reject:(error?:any)=>void ) => void = null)
		{
			...
			if (Weave.IS(relevantContext, WeavePromise))
			{
				...
				this.rootPromise = (relevantContext as WeavePromise<T>).rootPromise;
		...
		private rootPromise:WeavePromise<T>;
		protected relevantContext:Object;
		private result:T = undefined;
		private error:Error|string = undefined;
		private/* readonly */ handlers:WeavePromiseHandler<any, any>[] = []; // array of Handler objects
		private/* readonly */ dependencies:ILinkableObject[] = [];
		
		/**
		 * @return This WeavePromise
		 */
		public setResult(result:T):WeavePromise<T>
		{
			...
			var wp:WeavePromise<any> = Weave.AS(result, WeavePromise);
			if (wp)
			{
				wp._notify(this);
			}
			else if (WeavePromise.isThenable(result as any))
			{
				(result as any).then(this.setResult, this.setError);
			}
			else
			{
				this.result = result;
		...
		public static isThenable(obj:PromiseLike<any>):boolean
		{
		 ...
		 * @return This WeavePromise
		 */
		public setError(error:Error|string):WeavePromise<T>
		{
			if (Weave.wasDisposed(this.relevantContext))
				return this;
		...
		private callHandlers=(newHandlersOnly:boolean = false):void=>
		{
			...
			{
				var handler:WeavePromiseHandler<any, any> = this.handlers[i];
		...
		public then<U>(onFulfilled:(value:T) => (U|Promise<U>|WeavePromise<U>) = null, onRejected:(error:Error|string) => (U|Promise<U>|WeavePromise<U>) = null):WeavePromise<U>
		{
			if (Weave.wasDisposed(this.relevantContext))
				return this as any;
			
			var next:WeavePromise<U> = new WeavePromise<U>(this);
			this.handlers.push(new WeavePromiseHandler<any, any>(onFulfilled, onRejected, next));
		...
		private _notify(next:WeavePromise<any>):void
		{
			if (Weave.wasDisposed(this.relevantContext))
				return;
		...
		{
			var var_resolve:any, var_reject:any;
			var promise = new Promise(function(resolve, reject):void {
				var_resolve = resolve;
				var_reject = reject;
			});
