...
namespace weavejs.util
{
	export class WeavePromiseHandler<T, U>
	{
		...
		public constructor(onFulfilled:(value:T) => (U|Promise<U>|WeavePromise<U>), onRejected:(error:any) => (U|Promise<U>|WeavePromise<U>), next:WeavePromise<T>)
		{
			this.next = next;
		...
		public onError(error:Error):void
		{
			...
			try
			{
				if (this.onRejected != null)
					this.next.setResult(this.onRejected(error));
				else
