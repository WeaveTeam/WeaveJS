...
namespace weavejs.geom
{
	...
	export class BLGTree
	{
		 ...
		 */
		public insert(index:int, importance:number, x:number, y:number):void
		{
			// if this new point would have been in the previous traversal,
			// then the previous traversal is now invalid.
			if (importance >= this.previousTraversalMinImportance)
				this.previousTraversalMinImportance = -1; // reset this value so previous traversal won't be reused
		 ...
		 * @private
		 */
		private _insert(newNode:BLGNode):void
		{
			...
			var currentNode:BLGNode = this.rootNode;
			while (true)
			{
		 ...
		 */
		private operationStack:int[] = [];
		private nodeStack:BLGNode[] = [];
		private static /* readonly */ OP_VISIT:int = 0; // constant used with operationStack
		private static /* readonly */ OP_TRAVERSE:int = 1; // constant used with operationStack
		 ...
		 */
		public getPointVector(minImportance:number = 0, visibleBounds:Bounds2D = null):BLGNode[]
		{
			if (minImportance == this.previousTraversalMinImportance && this.previousTraversalVisibleBounds.equals(visibleBounds))
				return this.traversalVector; // avoid redundant computation
		 ...
		 */
		private traversalVector:BLGNode[] = [];
		/**
		 * This is the minImportance value from the last traversal.
		 * It can be used to avoid redundant traversal computations.
		 */
		private previousTraversalMinImportance:number = -1;
		/**
		 * This is the visibleBounds value from the last traversal.
		 * It can be used to avoid redundant traversal computations.
		 */
		private previousTraversalVisibleBounds:Bounds2D = new Bounds2D();

		private static assertTrue(bool:boolean):void
		{
			if (!bool)
				throw new Error("Assertion failed");
		}
		
		private static assertValid(node:BLGNode, assertNode:(node:BLGNode)=>void):void
		{
			if (!node)
				return;
		 ...
		 */
		public splitAtIndex(splitIndex:int):BLGTree
		{
