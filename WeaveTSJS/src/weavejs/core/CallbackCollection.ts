...
namespace weavejs.core
{
	...
	export class CallbackCollection implements ICallbackCollection, IDisposableObject
	{
		 ...
		 */
		constructor(preCallback:Callback = null)
		{
		 ...
		 */
		protected _preCallback:Callback = null;
		...
		public addImmediateCallback(relevantContext:Object, callback:Callback, runCallbackNow:boolean = false, alwaysCallLast:boolean = false):void
		{
			if (callback == null)
				return;
			...
			if (runCallbackNow)
			{
				...
				callback.apply(relevantContext || (callback as any)['this']);
				entry.recursionCount--;
			}
		}
			...
			for (var schedule:int = 0; schedule < 2; schedule++)
			{
				...
				for (var i:int = 0; i < this._callbackEntries.length; i++)
				{
					...
					if (entry.recursionCount == 0 || this._preCallback != null)
					{
						...
						entry.callback.apply(entry.context || (entry.callback as any)['this']);
