...
namespace weavejs.core
{
	...
	export class LinkableHashMap extends CallbackCollection implements ILinkableHashMap
	{
		...
		public getObjects<T>(filter:Class<T> = null, filterIncludesPlaceholders:boolean = false):T[]
		{
			return this.getList(true, filter, filterIncludesPlaceholders) as T[];
		}
		
		public toObject<T>(filter:Class<T & ILinkableObject> | string = null, filterIncludesPlaceholders:boolean = false):{[name:string]: T & ILinkableObject}
		{
			var obj:{[name:string]:ILinkableObject} = {};
			for (var name of this.getList(false, filter, filterIncludesPlaceholders) as string[] || [])
		...
		public toMap<T>(filter:Class<T> | string = null, filterIncludesPlaceholders:boolean = false):Map<string, T & ILinkableObject>
		{
			...
			for (var name of this.getList(false, filter, filterIncludesPlaceholders) as string[]|| [])
				map.set(name, this._nameToObjectMap[name] as any);
			return map;
		}
		...
		private getList(listObjects:boolean, filter:GenericClass|string, filterIncludesPlaceholders:boolean):string[]|ILinkableObject[]
		{
			...
			for (var i:int = 0; i < this._orderedNames.length; i++)
			{
				...
				else if (Weave.IS(object, filter as GenericClass))
				{
					result.push(listObjects ? object : name);
				}
				else if (filterIncludesPlaceholders)
				{
					var placeholder:LinkablePlaceholder<any> = Weave.AS(object, LinkablePlaceholder);
					if (!placeholder)
						continue;
					var classDef:GenericClass = placeholder.getClass();
					if (classDef === filter || Weave.IS(classDef.prototype, filter as GenericClass))
						result.push(listObjects ? object : name);
				}
			}
			return result;
		...
		public requestObject<T>(name:string, classDef:Class<T>|string, lockObject:boolean = false):T
		{
			...
			var result:ILinkableObject = this.initObjectByClassName(name, className, lockObject);
			...
			if (newStateArray != null)
			{
				...
				for (i = 0; i < newStateArray.length; i++)
				{
					...
					if (!DynamicState.isDynamicState(typedState, true))
						continue;
					objectName = (typedState as TypedState).objectName;
					className = (typedState as TypedState).className;
				...
				for (i = 0; i < newStateArray.length; i++)
				{
					...
					Weave.setState(object, (typedState as TypedState).sessionState, newObjects[objectName] || removeMissingDynamicObjects);
					if (removeMissingDynamicObjects)
						remainingObjects[objectName] = true;
					newNameOrder.push(objectName);
				}
