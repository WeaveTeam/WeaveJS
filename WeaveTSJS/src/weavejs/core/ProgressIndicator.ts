...
namespace weavejs.core
{
	...
	export class ProgressIndicator implements IProgressIndicator
	{
		...
		{
			var result:[any, number, string] = [] as any;
			var tasks = JS.mapKeys(this.map_task_progress);
			for (var task of tasks || [])
			{
		...
		public addTask(taskToken:Object|PromiseLike<any>, busyObject:ILinkableObject = null, description:string = null):void
		{
			...
			if (isNewTask && WeavePromise.isThenable(taskToken as PromiseLike<any>))
			{
				var remove = this.removeTask.bind(this, taskToken);
				(taskToken as PromiseLike<any>).then(remove, remove);
			}
