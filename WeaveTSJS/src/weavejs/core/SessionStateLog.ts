...
namespace weavejs.core
{
	...
	export class SessionStateLog implements ILinkableVariable, IDisposableObject
	{
		...
		constructor(subject:ILinkableObject, syncDelay:uint = 0)
		{
			...
			this._prevState = JS.copyObject(WeaveAPI.SessionManager.getSessionState(this._subject)) as TypedState; // remember the initial state
		...
		private _prevState:TypedState = null; // the previously seen session state of the subject
		...
		private immediateCallback():void
		{
			...
			{
				var state:TypedState = WeaveAPI.SessionManager.getSessionState(this._subject) as TypedState;
				var forwardDiff:TypedState = WeaveAPI.SessionManager.computeDiff(this._prevState, state) as TypedState;
		...
		private saveDiff(immediately:boolean = false):void
		{
			...
			var state:TypedState = sm.getSessionState(this._subject) as TypedState;
			...
			if (stepsRemaining > 0)
			{
				...
				while (stepsRemaining-- > 0)
				{
					...
					if (stepsRemaining == 0 && this.enableLogging.value)
					{
						...
						this._prevState = JS.copyObject(sm.getSessionState(this._subject)) as TypedState;
					}
		...
		{
			var h:any[] = this._undoHistory.concat();
			for (var i:int = 0; i < h.length; i++)
				h[i] = h[i].id;
			var f:any[] = this._redoHistory.concat();
