...
namespace weavejs.util
{
	 ...
	 * @author abaumann
	 */
	@Weave.classInfo({id: 'weavejs.util.ColorRamp'})
	export class ColorRamp extends LinkableVariable
	{
		public static /* readonly */ COLOR:string = 'color';
		public static /* readonly */ POSITION:string = 'position';
		
		constructor(sessionState:any = null)
		{
			super();
			super(null, this.verifyState, sessionState || ColorRamp.getColorRampByName("Blues").colors);
		}
		private verifyState(state:any):boolean
		{
			if (Weave.IS(state, String))
			{
				var reversed:boolean = false;
				
				// parse list of color values
				var colors:string[] = WeaveAPI.CSVParser.parseCSVRow(state as string);
				if (reversed)
					colors.reverse();
			...
			var type:GenericClass = StandardLib.getArrayType(state);
			if (type == Number)
			{
				var array:string[] = [];
				for (var number of state || [])
					array.push('0x' + StandardLib.numberToBase(number, 16, 6).toUpperCase());
				this.setSessionState(array);
				return false;
			}
			return type == String || type == Object || (Weave.IS(state, Array) && !state.length);
		}
		
		private _validateTriggerCount:uint = 0;
		...
		private validate():void
		{
			...
			this._colorNodes = [];
			for (var item of this.getSessionState() || [])
			{
				if (typeof item == 'object')
					this._colorNodes.push(new ColorNode(item[ColorRamp.POSITION], item[ColorRamp.COLOR]));
				else
					this._colorNodes.push(new ColorNode(this._colorNodes.length, StandardLib.asNumber(item)));
			}
			
			// if min,max positions are not 0,1, normalize all positions between 0 and 1
			var positions:number[] = ArrayUtils.pluck(this._colorNodes, ColorRamp.POSITION);
			var minPos:number = Math.min.apply(null, positions);
			var maxPos:number = Math.max.apply(null, positions);
			for (var node of this._colorNodes)
				node.position = StandardLib.normalize(node.position, minPos, maxPos);
			
			StandardLib.sortOn(this._colorNodes, "position");
		}
		
		public reverse():void
		{
			var array:any[] = Weave.AS(this.getSessionState(), Array);
			if (array)
		...
		public getColors():number[]
		{
			...
			for(var i:int = 0; i < this._colorNodes.length; i++)
				colors.push(this._colorNodes[i].color);
		 ...
		 * @return A color.
		 */
		public getColorFromNorm(normValue:number):number
		{
			this.validate();
			...
			var leftNode:ColorNode = this._colorNodes[leftIndex];
			var rightNode:ColorNode = this._colorNodes[rightIndex];

			// handle boundary conditions
			if (rightIndex == 0)
				return rightNode.color;
			if (rightIndex == this._colorNodes.length)
		 ...
		 */
		public getColor(value:number, min:number = 0, max:number = 1):number
		{
			var normValue:number = min == 0 && max == 1 ? value : StandardLib.normalize(value, min, max);
			return this.getColorFromNorm(normValue);
		}
		 ...
		 */
		public getHexColor(value:number, min:number = 0, max:number = 1):string
		{
			var normValue:number = min == 0 && max == 1 ? value : StandardLib.normalize(value, min, max);
			var color:number = this.getColorFromNorm(normValue);
			if (!isFinite(color))
				return null;
		...
		*/
		 ...
		 */
		public static getColorRampByName(rampName:string):{name:string, tags:string, colors:number[]}
		{
		 ...
		 * @return An Object with "name", "tags", and "colors" properties.
		 */
		public static findMatchingColorRamp(ramp:ColorRamp):{name:string, tags:string, colors:number[]}
		{
			var colors = ramp.getColors();
			for (var obj of ColorRamp.allColorRamps)
			if (StandardLib.compare(obj.colors, colors) == 0)
				return obj;
			return null;
		}
		
		public static /* readonly */ allColorRamps:Array<{name:string, tags:string, colors:number[]}> = [
